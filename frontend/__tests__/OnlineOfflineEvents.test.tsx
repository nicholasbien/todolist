...
describe('Online/Offline Event Handling', () => {
  ...
  test('online event triggers todos refresh', async () => {
    ...
    await waitFor(() => {
      ...
      expect(fetch).toHaveBeenCalledWith(expect.stringContaining('/todos'), expect.objectContaining({
  ...
  test('multiple online events do not cause race conditions', async () => {
    ...
    await waitFor(() => {
      ...
      expect(fetch).toHaveBeenCalledWith(expect.stringContaining('/todos'), expect.any(Object));
    });
