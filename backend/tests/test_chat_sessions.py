...
class TestPendingSessionEnrichment:
    ...
    @pytest.mark.asyncio
    async def test_pending_sessions_include_message_count(
        self, client, user_id, space_id
    ):
        """Pending sessions should include message_count."""
        session_id = await create_session(
            user_id, space_id, "Count test", todo_id="todo_count"
        )
        await append_message(session_id, user_id, "user", "First message")
        await append_message(
            session_id, user_id, "assistant", "Reply", agent_id="claude"
        )
    ...
    @pytest.mark.asyncio
    async def test_pending_sessions_include_recent_messages(
        self, client, user_id, space_id
    ):
        """Pending sessions should include recent_messages with all user messages since last assistant reply."""
        session_id = await create_session(
            user_id, space_id, "Preview test", todo_id="todo_preview"
        )
    ...
    @pytest.mark.asyncio
    async def test_pending_sessions_recent_messages_multiple(
        self, client, user_id, space_id
    ):
        """recent_messages should include all user messages since last assistant reply."""
        session_id = await create_session(
            user_id, space_id, "Multi test", todo_id="todo_multi"
        )
        await append_message(session_id, user_id, "user", "First question")
        await append_message(
            session_id, user_id, "assistant", "Answer", agent_id="claude"
        )
    ...
    @pytest.mark.asyncio
    async def test_pending_sessions_recent_messages_new_session(
        self, client, user_id, space_id
    ):
        """For new sessions with no assistant reply, all messages should be in recent_messages."""
        session_id = await create_session(
            user_id, space_id, "New multi test", todo_id="todo_new_multi"
        )
    ...
    @pytest.mark.asyncio
    async def test_pending_sessions_is_followup_when_agent_claimed(
        self, client, user_id, space_id
    ):
        """is_followup should be True when agent_id is set (agent previously claimed session)."""
        session_id = await create_session(
            user_id, space_id, "Followup test", todo_id="todo_followup"
        )
        await append_message(session_id, user_id, "user", "Initial request")
        await append_message(
            session_id, user_id, "assistant", "Here's my response", agent_id="claude"
        )
    ...
    @pytest.mark.asyncio
    async def test_pending_sessions_not_followup_for_unclaimed(
        self, client, user_id, space_id
    ):
        """is_followup should be False for unclaimed sessions (no agent_id)."""
        session_id = await create_session(
            user_id, space_id, "New test", todo_id="todo_new"
        )
...
class TestInterimFlag:
    ...
    @pytest.mark.asyncio
    async def test_interim_true_does_not_clear_needs_agent_response(
        self, client, user_id, space_id
    ):
        ...
        session_id = await create_session(
            user_id, space_id, "Interim test", todo_id="todo_interim1"
        )
    ...
    @pytest.mark.asyncio
    async def test_interim_false_clears_needs_agent_response(
        self, client, user_id, space_id
    ):
        ...
        session_id = await create_session(
            user_id, space_id, "Non-interim test", todo_id="todo_interim2"
        )
        await append_message(session_id, user_id, "user", "Help me")
        ...
        await append_message(
            session_id, user_id, "assistant", "Here is the answer", agent_id="claude"
        )
    ...
    @pytest.mark.asyncio
    async def test_interim_true_still_sets_unread_and_agent_id(
        self, client, user_id, space_id
    ):
        ...
        session_id = await create_session(
            user_id, space_id, "Interim flags test", todo_id="todo_interim3"
        )
        await append_message(session_id, user_id, "user", "Help me")
