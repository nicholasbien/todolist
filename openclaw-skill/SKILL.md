...
- `TODOLIST_API_URL` — The API base URL (default: `https://backend-openclaw.up.railway.app`)
...
If `TODOLIST_API_URL` is not set, default to `https://backend-openclaw.up.railway.app`.
...
curl -s -H "Authorization: Bearer $TODOLIST_AUTH_TOKEN" \
  "${TODOLIST_API_URL:-https://backend-openclaw.up.railway.app}/spaces" | jq '.'
```
...
```bash
curl -s -H "Authorization: Bearer $TODOLIST_AUTH_TOKEN" \
  "${TODOLIST_API_URL:-https://backend-openclaw.up.railway.app}/todos?space_id=SPACE_ID" | jq '.'
```
...
curl -s -X POST -H "Authorization: Bearer $TODOLIST_AUTH_TOKEN" \
  ...
  "${TODOLIST_API_URL:-https://backend-openclaw.up.railway.app}/todos" | jq '.'
```
...
curl -s -X PUT -H "Authorization: Bearer $TODOLIST_AUTH_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"text": "UPDATED_TEXT"}' \
  "${TODOLIST_API_URL:-https://backend-openclaw.up.railway.app}/todos/TODO_ID" | jq '.'
```
...
```bash
curl -s -X PUT -H "Authorization: Bearer $TODOLIST_AUTH_TOKEN" \
  "${TODOLIST_API_URL:-https://backend-openclaw.up.railway.app}/todos/TODO_ID/complete" | jq '.'
```
...
curl -s -X DELETE -H "Authorization: Bearer $TODOLIST_AUTH_TOKEN" \
  "${TODOLIST_API_URL:-https://backend-openclaw.up.railway.app}/todos/TODO_ID"
```
...
```bash
curl -s -H "Authorization: Bearer $TODOLIST_AUTH_TOKEN" \
  "${TODOLIST_API_URL:-https://backend-openclaw.up.railway.app}/journals?date=YYYY-MM-DD&space_id=SPACE_ID" | jq '.'
```
...
curl -s -X POST -H "Authorization: Bearer $TODOLIST_AUTH_TOKEN" \
  ...
  "${TODOLIST_API_URL:-https://backend-openclaw.up.railway.app}/journals" | jq '.'
```
...
```bash
curl -s -H "Authorization: Bearer $TODOLIST_AUTH_TOKEN" \
  "${TODOLIST_API_URL:-https://backend-openclaw.up.railway.app}/insights?space_id=SPACE_ID" | jq '.'
```
...
```bash
curl -s -H "Authorization: Bearer $TODOLIST_AUTH_TOKEN" \
  "${TODOLIST_API_URL:-https://backend-openclaw.up.railway.app}/export?data=todos&space_id=SPACE_ID&format=json"
```
...
```bash
curl -s -H "Authorization: Bearer $TODOLIST_AUTH_TOKEN" \
  "${TODOLIST_API_URL:-https://backend-openclaw.up.railway.app}/agent/sessions?space_id=SPACE_ID" | jq '.'
```
...
curl -s -X POST -H "Authorization: Bearer $TODOLIST_AUTH_TOKEN" \
  ...
  "${TODOLIST_API_URL:-https://backend-openclaw.up.railway.app}/agent/sessions" | jq '.'
```
...
```bash
curl -s -H "Authorization: Bearer $TODOLIST_AUTH_TOKEN" \
  "${TODOLIST_API_URL:-https://backend-openclaw.up.railway.app}/agent/sessions/SESSION_ID" | jq '.'
```
...
```bash
curl -s -H "Authorization: Bearer $TODOLIST_AUTH_TOKEN" \
  "${TODOLIST_API_URL:-https://backend-openclaw.up.railway.app}/agent/sessions/pending?space_id=SPACE_ID&agent_id=openclaw" | jq '.'
```
...
curl -s -X POST -H "Authorization: Bearer $TODOLIST_AUTH_TOKEN" \
  ...
  "${TODOLIST_API_URL:-https://backend-openclaw.up.railway.app}/agent/sessions/SESSION_ID/messages" | jq '.'
```
...
```bash
curl -s -H "Authorization: Bearer $TODOLIST_AUTH_TOKEN" \
  "${TODOLIST_API_URL:-https://backend-openclaw.up.railway.app}/agent/sessions/unread-todos?space_id=SPACE_ID" | jq '.'
```
...
```bash
curl -s -X DELETE -H "Authorization: Bearer $TODOLIST_AUTH_TOKEN" \
  "${TODOLIST_API_URL:-https://backend-openclaw.up.railway.app}/agent/sessions/SESSION_ID"
```
...
```bash
curl -s -H "Authorization: Bearer $TODOLIST_AUTH_TOKEN" \
  "${TODOLIST_API_URL:-https://backend-openclaw.up.railway.app}/todos/PARENT_TODO_ID/subtasks" | jq '.'
```
