...
### E2E Offline Sync Tests (Playwright)

**Run when changing `public/sw.js` or `context/OfflineContext.tsx`.** Requires both servers running.
...
These tests simulate real network offline/online toggling and verify:
- Creating todos offline → syncing when back online
- Completing/editing todos offline → syncing correctly
- Journal offline create/update → syncing on reconnect

**Test account:** `test@example.com` / `000000` (bypasses email verification)
