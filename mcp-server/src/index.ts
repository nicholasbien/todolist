...
class TodolistMCPServer {
  ...
  private spaceId(args: any): string {
    return args?.space_id || DEFAULT_SPACE_ID || undefined;
  }

  // ── Todos ──

  private async listTodos(args: any) {
    const params: any = {};
    ...
    if (todos.length === 0) return this.text('No todos found.');
  ...
  private async addTodo(args: any) {
    ...
    return this.text(
      `Added todo: "${todo.text}" [${todo.category}] (${todo.priority}) (ID: ${todo._id})`
    );
  }

  private async updateTodo(args: any) {
  ...
  private async completeTodo(args: any) {
  ...
  private async deleteTodo(args: any) {
    await api.delete(`/todos/${args.id}`);
    return this.text(`Deleted todo ${args.id}`);
  }

  private async reorderTodos(args: any) {
  ...
  private async listSpaces() {
    ...
    if (spaces.length === 0) return this.text('No spaces found.');
  ...
  private async listSpaceMembers(args: any) {
    const response = await api.get(`/spaces/${args.space_id}/members`);
    const members = response.data;
    if (!Array.isArray(members) || members.length === 0) return this.text('No members found.');
  ...
  private async inviteToSpace(args: any) {
  ...
  private async leaveSpace(args: any) {
  ...
  private async listCategories(args: any) {
    ...
    if (categories.length === 0) return this.text('No categories found.');
    return this.text(categories.join(', '));
  }

  private async addCategory(args: any) {
  ...
  private async renameCategory(args: any) {
    const params: any = {};
    const sid = this.spaceId(args);
    if (sid) params.space_id = sid;
  ...
  private async getJournal(args: any) {
    const params: any = {};
    ...
    if (Array.isArray(data)) {
      const lines = data.map((e: any) => `[${e.date}] ${e.text}`);
      return this.text(lines.join('\n\n'));
    }
  ...
  private async writeJournal(args: any) {
    const body: any = { date: args.date, text: args.text };
    const sid = this.spaceId(args);
    if (sid) body.space_id = sid;
  ...
  private async deleteJournal(args: any) {
  ...
  private async listSessions(args: any) {
    ...
    if (sessions.length === 0) return this.text('No chat sessions found.');
  ...
  private async createSession2(args: any) {
  ...
  private async claimSession(args: any) {
  ...
  private async releaseSession(args: any) {
  ...
  private async postToSession(args: any) {
    const body = {
  ...
  private async deleteSession2(args: any) {
